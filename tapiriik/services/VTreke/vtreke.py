from tapiriik.settings import WEB_ROOT, VTREKE_CLIENT_SECRET, VTREKE_CLIENT_ID, VTREKE_RATE_LIMITS
...
class VTrekeService(ServiceBase):
    ...
    def WebInit(self):
        params = {'scope':'activity:write,activity:read_all',
                  'client_id':VTREKE_CLIENT_ID,
                  'response_type':'code',
                  'redirect_uri':WEB_ROOT + reverse("oauth_return", kwargs={"service": "vtreke"}),
    ...
    def _requestWithAuth(self, reqLambda, serviceRecord):
        ...
        if time.time() > serviceRecord.Authorization.get("AccessTokenExpiresAt", 0) - 60:
            ...
            response = requests.post("https://www.strava.com/oauth/token", data={
                ...
                "client_id": VTREKE_CLIENT_ID,
                "client_secret": VTREKE_CLIENT_SECRET,
            })
    ...
    def RetrieveAuthorizationToken(self, req, level):
        ...
        params = {"grant_type": "authorization_code", "code": code, "client_id": VTREKE_CLIENT_ID, "client_secret": VTREKE_CLIENT_SECRET, "redirect_uri": WEB_ROOT + reverse("oauth_return", kwargs={"service": "vtreke"})}
    ...
    def RevokeAuthorization(self, serviceRecord):
        ...
        if resp.status_code != 204 and resp.status_code != 200:
            raise APIException("Unable to deauthorize VTreke auth token, status " + str(resp.status_code) + " resp " + resp.text)
    ...
    def DownloadActivityList(self, svcRecord, exhaustive=False):
        ...
        while True:
            ...
            try:
                ...
                raise APIException("Failed parsing vtreke list response %s - %s" % (resp.status_code, resp.text))
            ...
            for ride in reqdata:
                ...
                # VTreke doesn't handle "timer time" to the best of my knowledge - although they say they do look at the FIT total_timer_time field, so...?
    ...
    def DownloadActivity(self, svcRecord, activity):
        ...
        lap = Lap(stats=activity.Stats, startTime=activity.StartTime, endTime=activity.EndTime) # VTreke doesn't support laps, but we need somewhere to put the waypoints.
        activity.Laps = [lap]
        lap.Waypoints = []
        ...
        if "error" in ridedata:
            raise APIException("VTreke error " + ridedata["error"])
        ...
        for idx in range(0, waypointCt - 1):
            ...
            # When pausing, VTreke sends this format:
        ...
        if activity.CountTotalWaypoints():
            ...
            while not response.json()["activity_id"]:
                ...
                if response.json()["error"]:
                    ...
                    raise APIException("VTreke failed while processing activity - last status %s" % response.text)
    ...
    def DeleteCachedData(self, serviceRecord):
        cachedb.vtreke_cache.remove({"Owner": serviceRecord.ExternalID})
        cachedb.vtreke_activity_cache.remove({"Owner": serviceRecord.ExternalID})
