...
tapiriik.Init = function () {
  ...
  if (tapiriik.User !== undefined) {
    if (tapiriik.User.ConnectedServicesCount > 1) {
      tapiriik.UpdateCountdownTimer = setInterval(
        tapiriik.UpdateSyncCountdown,
        60000
      );
      tapiriik.RefreshCountdownTimer = setInterval(
  ...
  if (tapiriik.User !== undefined) {
    for (var i in tapiriik.ServiceInfo) {
      if (
        tapiriik.ServiceInfo[i].Connected &&
        tapiriik.ServiceInfo[i].RequiresConfiguration &&
        !tapiriik.ServiceInfo[i].Configured
      ) {
        tapiriik.ActivateSetupDialog(i);
        break; // we can nag them again if there's >1
      }
      if (
        tapiriik.User.AutoSyncActive &&
        tapiriik.ServiceInfo[i].Connected &&
        tapiriik.ServiceInfo[i].HasExtendedAuth &&
        !tapiriik.ServiceInfo[i].PersistedExtendedAuth
      ) {
...
tapiriik.AddressChanged = function () {
  ...
  for (var i = 0; i < tapiriik.PreviousURLComponents.length; i++) {
    if (
      ...
      components[i] != tapiriik.PreviousURLComponents[i]
    ) {
      break;
    }
    unchangedDepth = i + 1;
      ...
      if (tapiriik.ServiceInfo.dropbox.AccessLevel == "full") {
        if (unchangedDepth <= 2) {
          tapiriik.DropboxBrowserPath =
            tapiriik.ServiceInfo.dropbox.Config.SyncRoot;
          $.address.value(
            "configure/dropbox/setup" + tapiriik.DropboxBrowserPath
          ); // init directory, meh
...
tapiriik.SaveConfig = function (svcId, config, callback) {
  $.post(
    "/configure/save/" + svcId,
    { config: JSON.stringify(tapiriik.ServiceInfo[svcId].Config) },
    function () {
      $.address.value("");
...
tapiriik.OpenAuthDialog = function (svcId) {
  ...
  if (mode == "oauth") {
    if (tapiriik.ServiceInfo[svcId].NoFrame) {
      // this should never happen, but in case someone curious tries the URL
      window.location = tapiriik.ServiceInfo[svcId].AuthorizationURL;
      contents = $("<div><h1>Weeeeee</h1>(redirecting you right now)</div>");
    } else {
      contents = $("<iframe>")
        .attr("src", tapiriik.ServiceInfo[svcId].AuthorizationURL)
        .attr("id", svcId);
    }
  } else if (mode == "direct") {
...
tapiriik.OpenDeauthDialog = function (svcId) {
  var form = $(
    '<form><center><button id="disconnect" class="delete">Disconnect</button><button id="cancel" class="cancel">Never mind</button></center></form><h2>(nothing will be deleted)</h2>'
  );
  ...
  $("#disconnect", form).click(function () {
    ...
    $.ajax({
      url: "/auth/disconnect-ajax/" + svcId,
      type: "POST",
      success: function () {
        $.address.value("");
        window.location.reload();
...
tapiriik.CreateDirectLoginForm = function (svcId) {
  ...
  form.bind("submit", function () {
    ...
    $.post(
      "/auth/login-ajax/" + svcId,
      {
        username: $("#email", form).val(),
        password: $("#password", form).val(),
        persist: $("#persist", form).is(":checked") ? "1" : null,
      },
      function (data) {
        if (data.success) {
          $.address.value("");
          ...
          if (
            ...
            data.result.type == "renew_password" &&
            svcId == "garminconnect"
          ) {
            alert(
              "You need to visit connect.garmin.com directly to fix a problem with your account.\n\nOnce you're done, try logging in again."
            );
          }
          if (
            typeof data.result === "object" &&
            data.result.type == "locked" &&
            svcId == "garminconnect"
          ) {
            alert(
              "If you entered your Garmin Connect username instead of your email, try using your email. If that doesn't work, visit connect.garmin.com to double-check your login.\n\nOnce you're done, try logging in again."
            );
          }
          if (
            typeof data.result === "object" &&
            data.result.type == "non_athlete_account" &&
            svcId == "trainingpeaks"
          ) {
            alert(
              "It looks like you used a TrainingPeaks Coach account - you'll have to sign in with your individual account to continue."
            );
          }
          $(".error", form).hide();
...
tapiriik.OpenServiceConfigPanel = function (svcId) {
  ...
  $("button#save", configPanel).click(function () {
    ...
    tapiriik.ServiceInfo[svcId].Config.sync_private = $(
      "#sync_private",
      configPanel
    ).is(":checked");
    tapiriik.ServiceInfo[svcId].Config.auto_pause = $(
      "#auto_pause",
      configPanel
    ).is(":checked");
    ...
    $.post(
      "/configure/flow/save/" + svcId,
      { flowFlags: JSON.stringify(flowFlags) },
      function () {
        $.post(
          "/configure/save/" + svcId,
          { config: JSON.stringify(tapiriik.ServiceInfo[svcId].Config) },
          function () {
            ...
            setTimeout(function () {
              window.location.reload();
            }, 400); //would be possible to resolve the changes in JS to avoid a reload, I'll leave that for later
          }
        );
      }
    );
...
tapiriik.OpenDropboxConfigDialog = function () {
  var configPanel = $(
    '<form class="dropboxConfig"><h1>Set Up Dropbox Sync</h1>\
		...
		<button id="OK">Save</button><button id="cancel" class="cancel">Cancel</button></form>'
  ).addClass("dropboxConfig");

  if (tapiriik.ServiceInfo.dropbox.Config.UploadUntagged)
    $("#syncAll", configPanel).attr("checked", "");
  $("#format", configPanel).val(tapiriik.ServiceInfo.dropbox.Config.Format);
  $("#filename", configPanel).val(
    tapiriik.ConvertDropboxFilenameToDisplay(
      tapiriik.ServiceInfo.dropbox.Config.Filename
    )
  );
  ...
  if (tapiriik.ServiceInfo.dropbox.AccessLevel == "full") {
    ...
    $("#syncLocationPreamble", configPanel).html(
      "<label>Activities sync to:</label>"
    ); // more meh
...
tapiriik.OpenDropboxInfoDialog = function () {
  var infoPanel = $(
    '<div style="max-width:500px"><h1>You should know...</h1>\
...
tapiriik.SaveDropboxConfig = function () {
  if (
    tapiriik.DropboxBrowserPath !== undefined &&
    tapiriik.DropboxBrowserPath.length <= 1
  ) {
  ...
  tapiriik.ServiceInfo.dropbox.Config.UploadUntagged =
    $("#syncAll").is(":checked");
...
tapiriik.PopulateDropboxBrowser = function () {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    $('<a class="folder"/>')
      ...
      .attr("path", build)
      .appendTo(fstack)
      .click(tapiriik.DropboxBrowserNavigateDown);
  }
  ...
  if (
    tapiriik.DropboxBrowserPath == tapiriik.CurrentDropboxBrowserPath &&
  ...
  $("#folderList ul").animate({
    "margin-left":
      (tapiriik.DropboxNavigatingUp ? 1 : -1) * $("#folderList").width(),
  });

  if (tapiriik.OutstandingDropboxNavigate !== undefined)
    tapiriik.OutstandingDropboxNavigate.abort();
...
tapiriik.PopulateDropboxBrowserCallback = function (data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    $("<a>")
      .text(data[i].replace(tapiriik.DropboxBrowserPath, "").replace(/^\//, ""))
      .attr("path", data[i])
      .appendTo(li)
      .click(tapiriik.DropboxBrowserNavigateDown);
  }
...
tapiriik.OpenPaymentReclaimInitiatedDialog = function (email) {
  var form = $(
    '<center><h1>The email is on its way</h1>An email has been sent to <b><span class="email"></span></b> with a link to reclaim your payment.<br/>Don\'t have access to <b><span class="email"></span></b> any more? <a href="mailto:contact@tapiriik.com">Get in touch</a><br/><button id="acknowledge">I\'ll be waiting</button></center>'
  );
...
tapiriik.OpenPaymentReclaimCompletedDialog = function () {
  var form = $(
    '<center><h1>You\'re good to go!</h1>Your payment has been reclaimed &amp; associated with the services you are currently connected to, and any you connect in the future.<br/><button id="acknowledge">Great</button></center>'
  );
  $("#acknowledge", form).click(function () {
    $.address.value("");
  });
...
tapiriik.OpenPaymentPromoClaimCompletedDialog = function () {
  var form = $(
    '<center><h1>You\'re good to go!</h1>Your account has been set up for automatic synchronization! You can always transfer the promo code to another account at any time.<br/><button id="acknowledge">Great</button></center>'
  );
  $("#acknowledge", form).click(function () {
    $.address.value("");
    window.location.reload();
...
tapiriik.CreateServiceDialog = function (serviceID, contents) {
  ...
  if (serviceID != "tapiriik") {
    icon = $("<img>").attr(
      "src",
      tapiriik.StaticURL + "img/services/" + serviceID + "_l.png"
    ); // Magic URL :\
  } else {
    icon = $("<div>").addClass("logo inline").text("tapiriik");
  }
  popover = $("<div>")
    .addClass("dialogPopoverWrap")
    .append(tapiriik.CreatePopover(contents).css({ position: "relative" }));
...
tapiriik.CreatePopover = function (contents) {
  var popoverStruct = $(
    '<div class="popover"><div class="popoverOuterBorder"><div class="popoverArrow"><div class="popoverArrowInner"></div></div><div class="popoverInner"></div></div></div>'
  );
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $rootScope.$apply(function () {
        // Tie us into Angularland
        tapiriik.NextSync =
          data.NextSync !== null ? new Date(data.NextSync) : null;
        tapiriik.LastSync =
          data.LastSync !== null ? new Date(data.LastSync) : null;
...
tapiriik.FormatTimespan = function (spanMillis) {
  if (Math.abs(spanMillis / 1000) > 60 * 60) {
    return (
      Math.round(spanMillis / 1000 / 60 / 60) +
      " hour" +
      (Math.round(spanMillis / 1000 / 60 / 60) != 1 ? "s" : "")
    );
  } else if (Math.abs(spanMillis / 1000) > 60) {
    return (
      Math.round(spanMillis / 1000 / 60) +
      " minute" +
      (Math.round(spanMillis / 1000 / 60) != 1 ? "s" : "")
    );
  } else {
    return (
      Math.ceil(spanMillis / 1000) +
      " second" +
      (Math.ceil(spanMillis / 1000) != 1 ? "s" : "")
    );
  }
};
tapiriik.RefreshSyncCountdown = function () {
  ...
  if (tapiriik.SyncHash !== undefined) {
    ...
    if (delta > 0 || (tapiriik.NextSync === null && !tapiriik.Synchronizing)) {
      $("#syncButton").show();
      var inCooldown =
        new Date() - tapiriik.LastSync <= tapiriik.MinimumSyncInterval * 1000;
      ...
      if (!tapiriik.Synchronizing) {
        var waitTimeMessage = "";
        if (tapiriik.SynchronizationWaitTime > 60) {
          // Otherwise you'd expect a countdown, which this is generally not.
          waitTimeMessage =
            " (approx. " +
            tapiriik.FormatTimespan(tapiriik.SynchronizationWaitTime * 1000) +
            ")";
        }
        ...
        if (tapiriik.SynchronizationStep == "list") {
          sync_state_text =
            "Checking " +
            tapiriik.ServiceInfo[tapiriik.SynchronizationProgress].DisplayName;
        } else {
          sync_state_text =
            Math.round(tapiriik.SynchronizationProgress * 100) + "% complete";
        }
      }
      if (tapiriik.FastUpdateCountdownTimer === undefined) {
        tapiriik.FastUpdateCountdownTimer = setInterval(
          tapiriik.UpdateSyncCountdown,
          6000
        );
      }
    ...
    $(".syncButton")
      .toggleClass("active", sync_button_active)
      .attr(
        "title",
        sync_button_active
          ? "Synchronize now"
    ...
    if (sync_state_text != $(".syncButtonAttachment.left").text()) {
      ...
      $(".syncButtonAttachment.left").animate(
        { width: newWidth + "px" },
        150,
        function () {
          if (currentWidth < newWidth) {
    ...
    setTimeout(function () {
      $(".syncButtonBlock").animate({ opacity: 1 });
    }, 500);
  }
};
...
tapiriik.ShowDebugInfo = function () {
  ...
  if (tapiriik.User !== undefined) {
    infoPane.append(
      $("<div><b>User ID:</b> <tt>" + tapiriik.User.ID + "</tt></div>")
    );
    infoPane.append(
      $("<div><b>User TZ:</b> <tt>" + tapiriik.User.Timezone + "</tt></div>")
    );
  }
  infoPane.append(
    $("<div><b>System:</b> <tt>" + tapiriik.SiteVer + "</tt></div>")
  );
  infoPane.hide();
...
tapiriik.CycleLogo = function () {
  ...
  var img = $("<img>").attr(
    "src",
    tapiriik.StaticURL + "img/tapiriik-" + logo_variant + ".png"
  );
  $(".logo a").fadeOut(function () {
    $(this).remove();
    img.appendTo(
      $("<a>").attr("href", "/").prependTo($(".logo")).hide().fadeIn()
    );
  });
};
